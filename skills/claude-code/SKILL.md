...
13. **Long prompts get truncated in print mode** — prompts over ~3000 chars via `-p "..."` may be silently cut off. For complex multi-section instructions, either (a) write instructions to a temp file and use a short prompt referencing it, or (b) break into multiple sequential `claude -p` calls. Piped input (`cat file | claude -p`) can also truncate — prefer passing file path in the prompt over piping content.

## References

- `references/mundo-upgrade-workflow.md` — Reusable pattern for upgrading Mundo skill via Claude Code delegation

## Rules for Hermes Agents

## Hermes Coordination Mode

User preference: **Hermes coordinates, Claude Code codes.** ("你当好管家就行了")

Hermes identifies gaps, designs solutions, delegates to Claude Code with precise instructions, then verifies and syncs results. See `references/hermes-coordination-pattern.md` for the full workflow.
